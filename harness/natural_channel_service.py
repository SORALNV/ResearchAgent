...
class ChannelSetupResult:
    ...
    def message(self) -> str:
        ...
        result = (
            ...
            "以後は普通に相談し、`試して`・`実行して`・`これで提出しよう`のように指示できます。"
        )
        if self.warning:
            result += f"\n**注意:** {self.warning}"
        return compact_discord_markdown(result)


class NaturalConversationHandler:
    """One conversational turn that may also emit an executable action contract."""
    ...
    def __call__(self, ingress: DiscordIngressResult) -> NaturalConversationResponse:
        ...
        channel = self.registry.active(ingress.event.payload.get("conversation_id") or "")
        workspace = (
            self.workspace_root
    ...
    def _store(self):
        ...
        if store is None:
            raise RuntimeError("NaturalConversationHandler.store is not configured")
        return store

    def bind_store(self, store: Any) -> "NaturalConversationHandler":
        self.store = store
        return self
    ...
    def _build_prompt(
        ...
        for event in self._store.latest_events(
            work_session_id=ingress.route.work_session.work_session_id,
            limit=60,
        ):
...
可視回答の末尾に、内部制御用JSONを一つだけ付ける。これはユーザーには表示されない。
...
class NaturalChannelService:
    """Channel setup, ordinary conversation, execution, submission and archive."""
    ...
    def setup_channel(
        ...
        *,
        ...
        try:
            self.router.store.create_project(
                name=channel.subject,
                domain=channel.domain,
                project_id=channel.project_id,
                root_ref=(
                    f"discord://{location.guild_id}/{location.conversation_id}"
                ),
                metadata={
                    "channel_session": True,
                    "conversation_id": location.conversation_id,
                    "domain": channel.domain.value,
                    "subject": channel.subject,
                    "target_ref": channel.target_ref,
                },
            )
        except ConflictError:
            project = self.router.store.get_project(channel.project_id)
            if project.domain != channel.domain:
                raise
    ...
    def handle_message(
        ...
        *,
        ...
        if _explicit_submit_intent(text) and channel.domain == Domain.KAGGLE:
            ...
            if proposal is None:
                extras.append(
                    "**実行は開始していません。** 実行可能な条件・entrypoint・成果物がまだ確定していないため、もう一度具体化します。"
                )
            else:
                extras.append(
        ...
        final_message = compact_join([message, *extras])
    ...
    def status(
        ...
        *,
        ...
        job_text = ", ".join(f"{key} {value}" for key, value in sorted(counts.items())) or "なし"
    ...
    def _accept_and_enqueue(
        ...
        *,
        ...
        job_text = f" Job `{selected.job_id}`をキューへ追加しました。" if selected else ""
        return compact_discord_markdown(
            f"**実行開始:** `{proposal.subject_ref}`（{proposal.title}）を採用し、実装・smoke test・実験へ進めます。{job_text}結果が出たら同じチャンネルへ要点を返します。"
        )
...
def _extract_protocol(text: str) -> dict[str, Any]:
    for block in reversed(_fenced_blocks(text)):
        ...
        if not isinstance(value, Mapping):
            continue
        if "assistant_action" in value or "job_proposals" in value:
            return dict(value)
    return {}
