...
class CodexAppServerSettings:
    ...
    def from_environment(
        ...
        turn_timeout = _optional_positive_float(
...
class CodexThreadBindingStore:
    """Durably map ResearchAgent work scopes to Codex thread IDs."""
    ...
    def put(self, binding: CodexThreadBinding) -> CodexThreadBinding:
        with self._lock:
            ...
            existing = current.get(binding.binding_key)
            value = replace(
                binding,
                created_at=(existing.created_at if existing else binding.created_at),
                updated_at=utc_timestamp(),
            )
            current[value.binding_key] = value
    ...
    def _read_all(self) -> dict[str, CodexThreadBinding]:
        ...
        for key, raw in value.items():
            if not isinstance(raw, Mapping):
                continue
            try:
                binding = CodexThreadBinding.from_dict(raw)
...
class CodexAppServerClient:
    """Bidirectional JSONL client for the official `codex app-server` protocol."""
    ...
    def start(self) -> None:
        with self._start_lock:
            ...
            if (
                ...
                and self.process_factory is _default_process_factory
            ):
    ...
    def notify(self, method: str, params: Mapping[str, Any] | None = None) -> None:
        if not self.running:
            self.start()
        self._send({"method": method, "params": _json_dict(params or {})})
    ...
    def _read_stdout(self) -> None:
        ...
        try:
            ...
            self._fail_pending(
                f"Codex App Server exited: returncode={returncode}; {self.stderr_tail}"
            )
            self._expire_approvals(
                f"Codex App Server connection closed: returncode={returncode}"
            )
            with self._state_lock:
    ...
    def _handle_notification(self, message: Mapping[str, Any]) -> None:
        ...
        if method == "thread/started":
            ...
            if isinstance(thread, Mapping):
                ...
                parent_thread_id = _optional_text(thread.get("parentThreadId"))
                if thread_id and parent_thread_id:
                    with self._state_lock:
                        parent_session = self._thread_sessions.get(parent_thread_id)
                        if parent_session:
                ...
                if isinstance(items, list):
                    for item in items:
                        if not isinstance(item, Mapping) or item.get("type") != "agentMessage":
                            continue
                        text = item.get("text")
                        if isinstance(text, str) and text.strip() and text not in run.final_messages:
                            run.final_messages.append(text)
    ...
    def _fail_pending(self, message: str) -> None:
        ...
        for pending in values:
            pending.error = {"code": -32000, "message": message}
            pending.event.set()
...
class CodexAppServerRuntime:
    """Thread/turn facade used by Discord and existing harness providers."""
    ...
    def steer(
        ...
        *,
        ...
        if run is None:
            raise CodexAppServerBusy("this WorkSession has no active Discord Codex turn")
    ...
    def status(self, *, session_id: str) -> dict[str, Any]:
        ...
        bindings = self.bindings.list_for_session(session_id)
        return {
            "running": self.client.running,
            "threads": [item.to_dict() for item in bindings],
            "active_turn": (
                {
...
class CodexAppServerAgentExecutor:
    ...
    def run(
        ...
        *,
        ...
        returncode = 0 if result.status == "completed" else (
            130 if result.status == "interrupted" else 1
        )
        return self.invocation_cls(
            role=role,
            stage=stage,
...
def format_codex_event_for_discord(event: CodexRuntimeEvent) -> str | None:
    ...
    if method == "turn/completed":
        ...
        return f"Codex turn completed: status=`{status}` turn=`{event.turn_id or '?'}`"
    if method == "turn/plan/updated":
        ...
        if explanation:
            lines.append(explanation[:1000])
        lines.extend(steps)
        return "\n".join(lines)
    if method == "item/mcpToolCall/progress":
        return "Codex MCP progress: " + str(params.get("message") or "")[:1500]
    if method in APPROVAL_METHODS:
        reason = str(params.get("reason") or "no reason supplied")
        if method == "item/commandExecution/requestApproval":
            command = str(params.get("command") or "")
            return (
                "Codex command approval required\n"
                f"- approval: `{event.approval_ref}`\n"
                f"- command: `{command[:1200]}`\n"
                f"- reason: {reason}\n"
                "Use `/agent codex_approval` with accept, acceptForSession, decline, or cancel."
            )
        return (
            "Codex file-change approval required\n"
            f"- approval: `{event.approval_ref}`\n"
            f"- reason: {reason}\n"
            "Use `/agent codex_approval` with accept, acceptForSession, decline, or cancel."
        )
    if method == "item/started":
        ...
        if item_type == "commandExecution":
            return f"Codex command started: `{str(item.get('command') or '')[:1200]}`"
...
def _thread_id_from_response(response: Any) -> str:
    ...
    if not thread_id:
        raise CodexAppServerProtocolError("thread response did not contain thread.id")
    return thread_id
