...
class ChannelSessionConfig:
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "ChannelSessionConfig":
        domain = normalize_domain(data.get("domain") or data.get("mode") or "")
        if domain not in {Domain.RESEARCH, Domain.KAGGLE}:
            raise ValueError("channel session mode must be research or kaggle")
        conversation_id = snowflake(
            str(data.get("conversation_id") or data.get("channel_id") or ""),
            "conversation_id",
        )
        channel_id = snowflake(
            str(data.get("channel_id") or conversation_id),
            "channel_id",
        )
        guild_id = snowflake(str(data.get("guild_id") or "0"), "guild_id")
        parent = data.get("parent_channel_id")
        parent_id = snowflake(str(parent), "parent_channel_id") if parent else None
        subject = str(data.get("subject") or data.get("topic") or "").strip()
        if not subject:
...
class ChannelSessionRegistry:
    ...
    def get_by_conversation_id(
        ...
        key = snowflake(conversation_id, "conversation_id")
        with self._lock:
    ...
    def bootstrap(self, environ: Mapping[str, str]) -> None:
        """Import optional JSON and legacy static channel maps idempotently."""
            ...
            if isinstance(decoded, list):
                ...
                for channel_id, item in decoded.items():
                    if isinstance(item, Mapping):
                        ...
                        records.append(
                            {"channel_id": str(channel_id), "mode": str(item)}
                        )
            else:
                raise ValueError(
